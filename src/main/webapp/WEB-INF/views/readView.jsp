	...
	<div class="container">
		<div class="row shadow-lg">
			...
			<form role="form" name="readForm" method="get"
				class="col-sm-4 border border-secondary mt-5 mb-5">
				<div style="float: left;" class="mt-3">
				<a href="/diary?memberNum=${read.boardMemberNum }" class="main-a">
					<img alt=""
			...
			<div class="col-sm-3" style="text-align: left">
				<div class="mt-5">
					...
					<div style="overflow: auto; height: 140px; padding-left: 3px;"
						class="border">   <c:if test = "${read.content eq null }">Trip Diary ${read.content}
						</c:if>   
							${read.content }
						 </div>
					...
					<ol class="replyList" style="padding-left: 3px;">
						<c:forEach items="${replyList}" var="replyList" varStatus="loop">
							<div style="font-size: 14px;">
	            				<span style="color:gray"><fmt:formatDate value="${replyList.regdate}"
									pattern="yyyy-MM-dd HH:mm" /></span>&nbsp;
								<a data-toggle="modal" href="#reportReply${replyList.replyNum}" style="color: #f35e5e;">
									신고 </a>
								<!-- c:choose 예시 esle if 너낌? -->
								<c:if test="${memberVo.memberNum eq replyList.memberNum }">
							...
							<div style="margin-bottom:3%" class="">${replyList.nickname} : ${replyList.content}</div>
			...
			<div class="col-sm-2" style="text-align: right; padding-right: 0">
				...
				<%--당사자에 한해서만 삭제/수정이 가능하게 버튼 보여주기 --%>
						<c:if test = "${read.nickname eq authInfo.nickname }">
				<div>
